...
LOGS_DIR = "../logs"
RESULTS_DIR = "../results"
SCRIPT_EXTRACT_RTT = "../../scripts/extract_rtt.py"
...
def main():
    ...
    for duration_min, qlog in bbr_runs:
        csv_path = qlog.replace(".qlog", "_rtt.csv")
        
        # Extract CWND (if csv not existent yet)
        if not os.path.exists(csv_path):
            cmd_extract = [
                sys.executable, SCRIPT_EXTRACT_RTT,
                "--qlog", qlog,
                "--csv", csv_path
            ]
    ...
    # Output path
    output_pdf = os.path.join(RESULTS_DIR, f"rtt_comparison_outages_bbr.pdf")
    
    # Define Title
    title = f"RTT Comparison: BBR with Intermittency"

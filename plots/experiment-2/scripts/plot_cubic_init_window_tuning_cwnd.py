...
def main():
    ...
    for folder in subdirs:
        ...
        if match:
            ...
            if os.path.exists(qlog):
                groups[key].append((init_window, qlog))
    ...
    for cca, runs in groups.items():
        ...
        for init_window, qlog in runs:
            ...
            # Format Label
            label = f"Init Window: {init_window}"
            
            # Add to plotter inputs
            plot_inputs.append(f"{label}={csv_path}")
            temp_files.append(csv_path)

            if init_window in LIMITED_PLOT:
                limited_plot_inputs.append(f"{label}={csv_path}")
        ...
        # Define Title
        title = f"CUBIC Initial Window Tuning (CWND Comparison)"

        # 1. Plot full comparison
        # Output path
        output_pdf = os.path.join(RESULTS_DIR, f"cubic_init_window_tuning_cwnd_full.pdf")
        ...
        # 2. Plot limited graph for better overview
        # Output path
        output_pdf = os.path.join(RESULTS_DIR, f"cubic_init_window_tuning_cwnd_limited.pdf")
        ...
        # Append all input files
        for inp in limited_plot_inputs:
            cmd_plot.extend(["--input", inp])

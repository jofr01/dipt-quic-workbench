...
def main():
    ...
    parser.add_argument("--ns3-dir", required=True, help="Directory containing NS-3 CSVs")
    ...
    # Load Data
    df_ns3 = load_data(args.ns3_dir, "NS-3 (Emulation)")
    df_wb = load_data(args.wb_dir, "Workbench (Simulation)")
    
    full_df = pd.concat([df_ns3, df_wb])
    ...
    plt.title(f"Queue Dynamics Comparison ({args.capacity} Mbps Link)")
    plt.ylim(bottom=0)
    plt.xlim(left=0)

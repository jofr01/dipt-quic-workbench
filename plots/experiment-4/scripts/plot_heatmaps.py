...
def main():
    ...
    for folder in subdirs:
        ...
        if match:
            ...
            raw_data[(uplink_bps, ack_ratio, loss_rate)].append(val)

    data_points = []
    
    # Process grouped data to get mean and std
    for (up, ack, loss), vals in raw_data.items():
        mean_val = np.mean(vals)
        std_val = np.std(vals) if len(vals) > 1 else 0.0
        print(f"{up}bps / ACK {ack} / Loss {loss}%: Mean = {mean_val:.2f} Mbps, Std = {std_val:.4f} Mbps (over {len(vals)} runs)")
    ...
    for loss in sorted(df["Loss"].unique()):
        ...
        # Check if we are stochastic or not
        if loss > 0:
            plot_title = f"Averaged Goodput Overview (Loss {loss}%)"
            subset[["Uplink", "ACK", "Value", "Std"]].to_csv(csv_path, index=False)
        else:
            plot_title = f"Goodput Overview (Loss {loss}%)"
